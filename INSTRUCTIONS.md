# Agent & developer instructions — svelte-supabase-auth-ai-chat-template
...
2. **API module**: `src/lib/<feature>.ts` using `apiFetch` — see `api-fetch` skill
3. **Route**: `src/routes/` + entry in `src/routes.ts` with auth `conditions`
4. **UI**: Svelte components under `src/components/` as needed
...
- Token for API: `localStorage['x-auth-token']` (managed in `src/lib/auth.ts`).
...
```
src/lib/auth.ts           Supabase auth stores + signOut clears chat state
...
src/components/           ChatSidebar, ChatMarkdown, AppHeader, AppShell
